...
async function onSubmit() {
  ...
  try {
    // Appel à ton backend Prisma
    const response = await api.post('/users/login', {
      email: email.value
    })

    // On stocke l'utilisateur retourné par Prisma (id, email, name)
    localStorage.setItem('user', JSON.stringify(response.data))

    // Petite pause visuelle pour l'expérience utilisateur
    setTimeout(() => {
    ...
    errorMsg.value = "Impossible de se connecter. Vérifie ton serveur backend."
  } finally {
